...
import Selenium.hw6.PageObjects.LoginPage;
...
public class LoginPageHelper {
    ...
    private static LoginPageHelper loginPageHelperInstance;
    ...
    public void clickServiceLink(String linkText){
        Assert.assertTrue(loginPage.servicesDropdown.findElement(By.linkText(linkText)).isDisplayed(),
                String.format("Can't find element by link text: '%s'.", linkText));
        loginPage.servicesDropdown.findElement(By.linkText(linkText)).click();
    }

    public static LoginPageHelper getInstance(RemoteWebDriver driver, SoftAssert softAssert){
        if (loginPageHelperInstance == null){
            loginPageHelperInstance = new LoginPageHelper(driver, softAssert);
        }
        return loginPageHelperInstance;
    }
